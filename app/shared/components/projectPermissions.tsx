    ...
    useEffect(() => {
        if (project && project > 0) {
            const getAccounts = async () => {
                const response = await fetch(`http://${process.env.SERVER_HOST}:3001/projects/editors`, {
                    method: 'POST',
            ...
            const getInvited = async () => {
                const response = await fetch(`http://${process.env.SERVER_HOST}:3001/projects/invited`, {
                    method: 'GET',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ project_id: project })
                })
