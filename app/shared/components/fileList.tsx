                ...
                Array.isArray(files) && files.length > 0 && (
                    files.map((file) => (
                        <div key={file.object_id}>
                            <button
                                className="text-slate-900 dark:text-slate-200 bg-gray-400 dark:bg-slate-800 hover:bg-slate-400 dark:hover:bg-slate-700 transition-colors duration-200 rounded-lg text-xl my-4 px-4 py-2"
                                onClick={() => {router.push(`/item/${file.object_id}`);}}
                            >
                              <div className="flex flex-col">
                                <div className="flex flex-row">
                                  <svg className="w-5 h-5 text-slate-900 dark:text-slate-200  flex-shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
