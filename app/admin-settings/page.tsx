...
function Home() {
  ...
  useEffect(() => {
    if (databaseExists === 2) {
      const getDatabaseExists = async () => {
        const response = await fetch(`http://${process.env.SERVER_HOST}:3001/database/exists`, {
          method: "GET",
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch(`http://${process.env.SERVER_HOST}:3001/users/getUsers`);
  ...
  const confirmSetupDatabase = async () => {
    const response = await fetch(`http://${process.env.SERVER_HOST}:3001/database/create`, {
      method: "GET",
      headers: { 'Content-Type': 'application/json' }
    });
  ...
  const handleDisableUser = async (uid: string) => {
    try {
      ...
      await fetch(`http://${process.env.SERVER_HOST}:3001/users/updateStatus`, {
        method: "PUT",
        headers: {
